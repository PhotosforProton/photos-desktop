...
// Local vendor of cloud-filter 0.0.6, with one change from upstream: the six
// callback trampolines below reported a filter error to the OS with
// `command::X::fail(..).unwrap()`. These run on the OS's `extern "system"` callback
// threads, so if the failure report itself is rejected (typically because the OS
// already cancelled or timed out the request) the unwrap panics, and a panic across
// that FFI boundary aborts the whole process. A download that fails must not crash
// the app, so each is now `let _ = command::X::fail(..)`: still report the failure,
// but never abort if the report cannot be delivered.
...
pub unsafe extern "system" fn fetch_data<T: SyncFilter + 'static>(
    ...
    if let Some(filter) = filter_from_info::<T>(info) {
        ...
        let _ = command::Write::fail(connection_key, transfer_key, e);
    }
}
...
pub unsafe extern "system" fn validate_data<T: SyncFilter + 'static>(
    ...
    if let Some(filter) = filter_from_info::<T>(info) {
        ...
        let _ = command::Validate::fail(connection_key, transfer_key, e);
    }
}
...
pub unsafe extern "system" fn fetch_placeholders<T: SyncFilter + 'static>(
    ...
    if let Some(filter) = filter_from_info::<T>(info) {
        ...
        let _ = command::CreatePlaceholders::fail(connection_key, transfer_key, e);
    }
}
...
pub unsafe extern "system" fn notify_dehydrate<T: SyncFilter + 'static>(
    ...
    if let Some(filter) = filter_from_info::<T>(info) {
        ...
        let _ = command::Dehydrate::fail(connection_key, transfer_key, e);
    }
}
...
pub unsafe extern "system" fn notify_delete<T: SyncFilter + 'static>(
    ...
    if let Some(filter) = filter_from_info::<T>(info) {
        ...
        let _ = command::Delete::fail(connection_key, transfer_key, e);
    }
}
...
pub unsafe extern "system" fn notify_rename<T: SyncFilter + 'static>(
    ...
    if let Some(filter) = filter_from_info::<T>(info) {
        ...
        let _ = command::Rename::fail(connection_key, transfer_key, e);
    }
}
